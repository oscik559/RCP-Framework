#!/usr/bin/env python3
"""
AGENTIC DATABASE INSPECTION UTILITY

Inspect and analyze the agentic.db database to understand workflow execution.
Shows goals, strategies, functions, inputs/outputs, and data flow connections.

Usage:
    python utilities/inspect_agentic_db.py

This tool traces the complete execution of the workflow including:
- Goal definitions
- Strategy selection and success
- Function execution sequence
- Input parameters for each function
- Output values from each function
- Data flow validation (which outputs feed into which function inputs)
"""
...
def query_db(db_path, sql):
    """Execute query on database and return results as list of dicts"""
    try:
...
print_subsection("4. DATA FLOW VALIDATION (Output → Input Connections)")
...
if strategy_id_list:
    ...
    for i in range(len(functions_ordered) - 1):
        ...
        print(f"\n  {current['FunctionName']} → {next_func['FunctionName']}")
...
if strategy_id_list:
    result = strategies[0] if strategies else {}
