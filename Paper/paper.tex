...
To illustrate the execution flow concretely, we trace a representative query through the \emph{Simple Lookup} strategy. The query targets a specific engineering attribute for an electronic connector component; proprietary identifiers are masked.
...
Against a dataset of ~50 catalog pages containing 1,628 product variants across 168 families for Case~I, we compared the RCP system against the baselines for a set of 50 human-annotated engineering queries.
...
\begin{table}[h]
    ...
    \begin{tabular}{@{}lrrr@{}}
        ...
        \midrule
        Answer Correctness              & 52.0\% & 82.0\% & \textbf{76.0\%} \\
        Citation Accuracy               & 100.0\% & 96.0\% & \textbf{78.0\%} \\
        Unit Fidelity                   & 76.0\% & 92.0\% & \textbf{78.0\%} \\
        Hallucination Rate $\downarrow$ & 48.0\% & 18.0\% & \textbf{2.0\%} \\
        \bottomrule
    \end{tabular}
...
The B1$\rightarrow$B2 transition reveals that the dominant failure mode in Naive RAG is retrieval fragmentation: chunks rarely contain complete specification records, causing the LLM to hallucinate values for missing fields (hallucination rate 48\%). Structured SQL retrieval reduces this to 18\% by delivering complete product records with preserved field semantics, lifting answer correctness from 52\% to 82\%. Residual B2 errors fall into two classes: keyword-search failures on semantic queries without explicit product codes (where LIKE-pattern fallback retrieves irrelevant records), and generation errors when multiple products are returned and the LLM conflates their attributes.

The B2$\rightarrow$B3 transition shows a different trade-off. Answer correctness decreases slightly (82\%$\rightarrow$76\%) while hallucination is nearly eliminated (18\%$\rightarrow$2\%). The correctness drop reflects explicit failures: when validation gates determine that retrieved evidence is insufficient or unit checks fail, the RCP returns a qualified non-answer rather than fabricating a plausible value. This ``verify-then-summarize'' behaviour is the intended design outcome---in engineering contexts, an explicit failure is preferable to a confident but incorrect output. The residual 2\% hallucination rate occurs solely in the LLM synthesis step (the final \texttt{func\_analyse\_data} call), where validated attributes are converted to natural language.
...
\input{appendix/prompts_improved}                      % Will be Appendix A  
% \input{appendix/sample_query_sets}            % Will be Appendix B
\input{appendix/sample_query_sets_corrected}            % Will be Appendix B
