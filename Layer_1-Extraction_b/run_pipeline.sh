...
PDF_PATH="Layer_1-Extraction_b/${PDF_FILE}"
START_PAGE=${1:-1}
END_PAGE=${2:-10}
...
Path('Layer_1-Extraction_b/data/png_pages').mkdir(parents=True, exist_ok=True)
Path('Layer_1-Extraction_b/data/pdf_pages').mkdir(parents=True, exist_ok=True)
...
for page_num in range($START_PAGE - 1, min($END_PAGE, total_pages)):
    ...
    png_path = f'Layer_1-Extraction_b/data/png_pages/page_{page_num + 1:03d}.png'
    pix.save(png_path)
    ...
    pdf_path = f'Layer_1-Extraction_b/data/pdf_pages/page_{page_num + 1:03d}.pdf'
...
uv run python 2_detect_headers_footers.py --pdf ../Layer_1-Extraction_b/$PDF_FILE
cd ..

echo ""
echo "✅ Pipeline setup complete!"
echo "Next steps:"
echo "  - Run table detection: uv run python Layer_1-Extraction_b/3_detect_tables.py --pdf Layer_1-Extraction_b/$PDF_FILE --page <PAGE>"
echo "  - Run product extraction: uv run python Layer_1-Extraction_b/4_extract_product.py --page <PAGE> --test"
